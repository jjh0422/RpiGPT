...
def remove_silence(audio_path, top_db=30):
    ...
    if len(non_silent_intervals) == 0:
        return None, 0, 0  # 무음 파일이면 None 반환
...
def recognize_speech_from_wav(wav_file_path):
    ...
    return text, rtf, vad_duration, vad_time
    ...
    with open(output_text_file, 'w', encoding='utf-8') as f:
        for filename in os.listdir(folder_path):
            if filename.endswith(".wav"):
                ...
                recognized_text, rtf, vad_duration, vad_time = recognize_speech_from_wav(wav_file_path)
                f.write(f"{file_name_without_extension} {recognized_text}\n")  # RTF 값 제거

                if rtf > 0:  # 유효한 RTF 값만 합산
                    total_rtf += rtf
                    total_vad_time += vad_time
                    file_count += 1
